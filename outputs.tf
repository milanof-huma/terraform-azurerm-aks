...
output "outbound_ips" {
  value = length(azurerm_kubernetes_cluster.main.network_profile) > 0 ? element(concat(azurerm_kubernetes_cluster.main.network_profile.0.load_balancer_profile, [""]), 0) : null
}
